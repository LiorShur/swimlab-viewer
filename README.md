...
By default the summary, correction points, and drills are generated by simple
rules in the page. Add `--narrate` to have **Claude** write that interpretation
layer instead — reasoning across the whole metric vector for a more tailored read:
...
python src/build.py --data sessions.json --out dashboard.html
#   --narrate-model claude-opus-5   (default; try claude-sonnet-5 / claude-haiku-4-5 for less cost)
```

Important boundaries (this is a pre‑validation research prototype):

- **The pipeline stays deterministic.** Claude is never in the measurement loop —
  the metrics, the lifter/rotator classification, and the plots are always the
  analysis library's. `--narrate` only rewrites the *interpretation* prose
  (`src/narrate.py`), leaving `data.json`'s numbers untouched.
- **Grounded, not free‑form.** The model is handed a fixed table of the computed
  metrics and constrained by a JSON schema so every correction cites the metric
  it rests on; it is told never to invent or recompute a value, and that
  everything is illustrative and hypothesis‑based, not validated.
- **Graceful fallback.** No key, no `anthropic` package, or an API error → export
  prints a notice and ships the rule‑based text. The viewer reads `data.json`'s
  optional `narrative` field when present (badged *AI‑generated*) and otherwise
  falls back automatically — so nothing breaks either way. One call per swimmer,
  a few thousand tokens; cost is well under a cent each.
